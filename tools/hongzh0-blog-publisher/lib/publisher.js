...
<meta property="og:type" content="article"><meta property="og:locale" content="zh_CN"><meta property="og:site_name" content="hongzh0's Blog"><meta property="og:title" content="${escapeHtml(post.title)}"><meta property="og:description" content="${escapeHtml(post.summary)}"><meta property="og:url" content="${escapeHtml(canonical)}"><meta property="og:image" content="${escapeHtml(OG_IMAGE)}"><meta property="og:image:alt" content="hongzh0 的个人照片"><meta property="og:image:width" content="960"><meta property="og:image:height" content="962"><meta property="article:published_time" content="${post.date}">
...
<main id="main"><header class="article-header wrap"><div class="post-meta"><span>${escapeHtml(post.category)}</span><time datetime="${post.date}">${displayDate(post.date)}</time><span>${post.minutes} 分钟阅读</span></div><h1>${escapeHtml(post.title)}</h1><p class="article-lead">${escapeHtml(post.summary)}</p></header>
<div class="featured-visual article-cover wrap"><span class="visual-grid" aria-hidden="true"></span><span class="visual-orbit orbit-one" aria-hidden="true"></span><span class="visual-orbit orbit-two" aria-hidden="true"></span><span class="visual-center" aria-hidden="true">${escapeHtml(post.coverText)}</span><span class="visual-caption" aria-hidden="true">SECURITY RESEARCH · ${escapeHtml(post.date)}</span></div>
<details class="mobile-toc wrap"><summary>文章目录</summary><nav aria-label="移动端文章目录">${tocLinks}</nav></details><div class="article-layout" id="article"><aside class="article-toc" aria-label="文章目录"><span>CONTENTS</span>${tocLinks}</aside><article class="article-content">${rendered.html}<div class="article-end">最后更新于 ${displayDate(post.date)} · hongzh0's Blog</div>${articleNavigation(options.previous, options.next)}</article></div></main>

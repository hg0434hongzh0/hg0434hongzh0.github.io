...
function fixture() {
  ...
  fs.writeFileSync(path.join(root, 'assets', 'style.css'), '/* source css */\nbody { color: red; }\n');
  fs.writeFileSync(path.join(root, 'assets', 'main.js'), '// source js\nconst answer = 40 + 2;\nconsole.log(answer);\n');
  fs.writeFileSync(path.join(root, 'assets', 'article-crypto.js'), '// crypto source\nwindow.cryptoReady = true;\n');
  fs.mkdirSync(path.join(root, 'assets', 'fonts', 'files'), { recursive: true });
  fs.writeFileSync(path.join(root, 'assets', 'fonts', 'font-face.css'), '/* local fonts */\n@font-face { font-family: Test; src: url(./files/test.woff2); }\n/* unused font */\n@font-face { font-family: Unused; src: url(./files/unused.woff2); unicode-range: U+10FFFF; }\n');
  fs.writeFileSync(path.join(root, 'assets', 'fonts', 'files', 'test.woff2'), 'test-font');
  fs.writeFileSync(path.join(root, 'assets', 'fonts', 'files', 'unused.woff2'), 'unused-font');
...
try {
  ...
  assert.match(article, /assets\/fonts\/font-face\.css\?v=[a-f0-9]{12}/);
  ...
  const outputStyle = fs.readFileSync(path.join(root, 'dist', 'assets', 'style.css'), 'utf8');
  const outputMain = fs.readFileSync(path.join(root, 'dist', 'assets', 'main.js'), 'utf8');
  const outputCrypto = fs.readFileSync(path.join(root, 'dist', 'assets', 'article-crypto.js'), 'utf8');
  const outputFonts = fs.readFileSync(path.join(root, 'dist', 'assets', 'fonts', 'font-face.css'), 'utf8');
  assert.doesNotMatch(outputStyle, /source css/);
  assert.doesNotMatch(outputMain, /source js/);
  assert.doesNotMatch(outputCrypto, /crypto source/);
  assert.doesNotMatch(outputFonts, /local fonts/);
  assert.match(outputStyle, /body\{color:red\}/);
  assert.match(outputMain, /console\.log/);
  assert.match(outputCrypto, /cryptoReady/);
  assert.match(outputFonts, /@font-face\{/);
  assert.match(outputFonts, /test\.woff2/);
  assert.doesNotMatch(outputFonts, /unused\.woff2/);
  assert.ok(fs.existsSync(path.join(root, 'dist', 'assets', 'fonts', 'files', 'test.woff2')));
  assert.ok(!fs.existsSync(path.join(root, 'dist', 'assets', 'fonts', 'files', 'unused.woff2')));
  assert.match(fs.readFileSync(path.join(root, 'assets', 'style.css'), 'utf8'), /source css/);

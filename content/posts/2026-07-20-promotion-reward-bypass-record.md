...
category: 漏洞分析
summary: "复盘 360 安全中心、潮汐 App、长亭相关产品中的账号生命周期绕过，并补充 new-api 的验证结论：其已对用户名和邮箱做重复性校验，未形成可稳定利用的 0day。"
...
下面按实际挖掘过程整理 360 安全中心 / Quake、潮汐 App、长亭相关产品，以及 new-api 场景中的横向拓展思路。

## 一、360 安全中心鉴权逻辑缺陷导致的推广奖励重置
...
360 安全中心允许账号绑定多种找回与登录因子。为了避免用户同时解绑全部双因素导致账号丢失，邮箱在流程中通常采用“换绑”而不是直接解绑的设计。问题出现在手机号与邮箱、主账号与新账号之间的状态迁移：手机号从旧账号解绑后，可继续作为验证码登录因子触发新账号流程；随后旧账号又可以重新绑定该手机号，从而形成循环。

### 2. 复现过程

1. 在 360 安全中心使用手机号 A 注册或绑定账号。下图为已注册账号的绑定状态示意。

   ![360 安全中心绑定状态](/assets/posts/promotion-reward-bypass-record/1766725262287-f72a487e-0d77-4607-bd0f-606f38025a51.png)
...
6. 回到 360 安全中心登录账号 1，将账号 2 当前使用的手机号 A 强制绑定回账号 1。
...
## 二、基于相同思路的横向拓展：潮汐 App 推广奖励问题

### 1. 思路

潮汐 App 的问题与上一节类似：一个可登录途径就可以视为一个独立身份因子。如果平台把“新用户权益”绑定在这些可切换的登录因子上，而没有稳定合并到同一用户主体，就会产生重复领取空间。

![潮汐 App 登录因子示意 1](/assets/posts/promotion-reward-bypass-record/5f05283fd75f559dcffe6e5c375d57db.jpg)

![潮汐 App 登录因子示意 2](/assets/posts/promotion-reward-bypass-record/7f61e2cdb62b805688394ae54ccbd152.jpg)

从页面可以看到，当前场景中存在 7 个可用身份因子。

![潮汐 App 身份因子列表](/assets/posts/promotion-reward-bypass-record/9c18912643f24a7bb8688c2b18b82279.jpg)
...
## 三、途径延伸类推广漏洞：长亭相关产品通用场景

### 1. 思路

长亭相关产品上线测试阶段曾出现过更直接的推广奖励问题：A 账号注册并分享邀请链接，B 账号通过邀请链接注册后给 A 返现；随后 B 账号注销并重新注册，循环触发积分。该问题在内部测试阶段已有一定防护。

![长亭推广规则示意](/assets/posts/promotion-reward-bypass-record/image-20260705104323601.png)
...
长亭侧反馈该项目主要面向内部 SRC 场景，后续补偿了一定中转站积分。

## 四、基于同类思路的 new-api 验证记录

在 new-api 场景中，也可以沿着“账号生命周期 + 邀请奖励 + 登录因子复用”的组合思路做验证。经过实际检查，new-api 已对用户名和邮箱做了重复性校验，因此这里**未形成可稳定利用的 0day**。
...
![new-api 场景截图 1](/assets/posts/promotion-reward-bypass-record/20260720104616105.png)

![new-api 场景截图 2](/assets/posts/promotion-reward-bypass-record/20260720104610614.png)
